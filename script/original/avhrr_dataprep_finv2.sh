...
if [ $# -eq 4 ]; then                    
  ...
  sourceString="$satSourceA,$satSourceB,$shipSource,$buoySource,$argoSource,$iceSource"
  
else

  sourceString="$satSourceA,$shipSource,$buoySource,$argoSource,$iceSource"
...
satellitePrep() {
        ...
        # count how many satellite files on the analysis day
          ...
          numfiles=$(ls $DATA_FINAL_INPUT/avhrr8day/$curYear12/*$curDate12* | wc -w )
          echo  $numfiles "satellite observation files on the analysis day $curYear12.$curJulian12"

          if [ $numfiles == 0 ]
          then
          echo "RED-STOP: No satellite observation files on the analysis day $curYear12.$curJulian12"
          exit 
          fi

          if [ $numfiles -lt 3 ]
          then
          echo "YELLOW-WARNING: There are less than three satellite observation files on the analysis day $curYear12.$curJulian12"
          fi
  ...
  ls $DATA_FINAL_INPUT/avhrr8day/$curYear12/mcsst_mta_d"$curDate12"_s* 
  ls $DATA_FINAL_INPUT/avhrr8day/$curDate12M1Year/mcsst_mta_d"$curDate12M1"_s2* 
  ls $DATA_FINAL_INPUT/avhrr8day/$curDate12P1Year/mcsst_mta_d"$curDate12P1"_s0* 

  cat $DATA_FINAL_INPUT/avhrr8day/$curYear12/mcsst_mta_d"$curDate12"_s* >> $satSSTCatObs
  cat $DATA_FINAL_INPUT/avhrr8day/$curDate12M1Year/mcsst_mta_d"$curDate12M1"_s2* >> $satSSTCatObs
  cat $DATA_FINAL_INPUT/avhrr8day/$curDate12P1Year/mcsst_mta_d"$curDate12P1"_s0* >> $satSSTCatObs
  ...
  ls $DATA_FINAL_INPUT/avhrr8day/$curYear12/mcsst_mtb_d"$curDate12"_s* 
  ls $DATA_FINAL_INPUT/avhrr8day/$curDate12M1Year/mcsst_mtb_d"$curDate12M1"_s2* 
  ls $DATA_FINAL_INPUT/avhrr8day/$curDate12P1Year/mcsst_mtb_d"$curDate12P1"_s0* 

  cat $DATA_FINAL_INPUT/avhrr8day/$curYear12/mcsst_mtb_d"$curDate12"_s* >> $satSSTCatObs
  cat $DATA_FINAL_INPUT/avhrr8day/$curDate12M1Year/mcsst_mtb_d"$curDate12M1"_s2* >> $satSSTCatObs
  cat $DATA_FINAL_INPUT/avhrr8day/$curDate12P1Year/mcsst_mtb_d"$curDate12P1"_s0* >> $satSSTCatObs
...
if [ -e  $seaIce ]; then
    ...
    if [ "$seaIceFileDate" != "$curDate12" ]; then
      echo 'YELLOW-WARNING: date in the header does not match today'
      echo 'analysis day curDate=' $curDate12' and seaIceFileDate='$seaIceFileDate
    fi
	...
	oiEOF
        ...
        echo 'YELLOW-WARNING: error found in seaice data.'$curDate12  
      fi
    else
      echo 'YELLOW-WARNING: No seaIceFileDate' $seaIceFileDate
    fi # end $seaIceFileDate != ""
  else
    echo 'YELLOW-WARNING: No ice data for analysis day' $curDate12
fi # end [ -e  $seaIce]
}
...
convertMedianToSST() {
  ...
  while [ "$dayCnt" -le "7" ]; do
    ...
    if [ "$interval" -lt "0" ]; then
      TM0=`sh $UTIL/finddate.sh $curDate_ice d$interval`
  ...
  if [ $iceConDays != 0 ]; then
	...
	$quarterMaskExtend
	$iceMask
	$iceFrzPnt.$curMonth_ice$curDay_ice
	$iceCon7day
	$iceConMed
	$iceSSTGrads
	$iceSobsOut
	$covNameIce
	$curYear_ice $curMonth_ice $curDay_ice $iceConDays
	$covIce
	$covIce2
	sstEOF
