...
# jday_19990514 = datetime.date(1999, 5 , 14)
jday_20170624 = datetime.date(2017, 6 , 24)
  ...
  # daily_sst_num_pf2oi_final.fill(0)
    ...
    for (id_fn, fn) in enumerate(fns, start=0):
      ...
      # daily_sst_num_pf2oi_final[:, :, id_fn] += daily_sst_num_pf2oi_1rec
        ...
        daily_sst_avg_pf2oi_1rec  = np.divide(daily_sst_avg_pf2oi_1rec , daily_sst_num_pf2oi_1rec   \
            , where=(daily_sst_num_pf2oi_1rec  > 0.9))
        daily_sst_avg_pf2oi_1rec  = xr.where(daily_sst_num_pf2oi_1rec ==0, -9999, daily_sst_avg_pf2oi_1rec -273.15)

      # da_daily_sst_avg = xr.DataArray(data=np.float32(daily_sst_avg_pf2oi_1rec.T)  \
      #     , dims=['lat', 'lon'], coords={'lat': lat_oi, 'lon': lon_oi}, name='sst'  \
      #     , attrs={'long_name':'sst binned average', 'units':'degC', '_FillValue':-9999})
      # da_daily_sst_num = xr.DataArray(data=np.int32  (daily_sst_num_pf2oi_1rec.T)  \
      ...
      # ds_daily.attrs = {}
      # if id_fn == 0: 
      #     fn_daily = str_date+'_day_PFV5.3-2OI.nc'
      # else:
      #     fn_daily = str_date+'_night_PFV5.3-2OI.nc'
      # ds_daily.to_netcdf(DIR_PF+'/PF2OI/'+fn_daily, encoding={'lat': {'_FillValue': None}, 'lon': {'_FillValue': None}})


        # to 2D OI-grid binary
        if id_fn == 0:
            fn_daily_bin_2d = str_date+'_day_PFV5.3-2OI_2D.bin'
        else:
            fn_daily_bin_2d = str_date+'_night_PFV5.3-2OI_2D.bin'

        fl = open(DIR_PF+'/PF2OI/'+fn_daily_bin_2d, 'wb')
        fl.write(struct.pack('>i', 4147212))
        fl.write(struct.pack('>iii', jday.year, jday.month, jday.day))
        fl.write(np.float32(daily_sst_avg_pf2oi_1rec.T).byteswap())
        fl.write(struct.pack('>i', 4147212))
        fl.close()

        # to 1D super-obs
        if id_fn == 0:
            fn_daily_bin_1d = str_date+'_day_PFV5.3-2OI_1D.bin'
        else:
            fn_daily_bin_1d = str_date+'_night_PFV5.3-2OI_1D.bin'

        ctype = 'pfv5.3  '
        fort.write_1d_sat_sst_super_obs(DIR_PF+'/PF2OI/'+fn_daily_bin_1d  \
            , np.float32(daily_sst_avg_pf2oi_1rec), np.int32(daily_sst_num_pf2oi_1rec), oi_mask_fort, jday.year  \
