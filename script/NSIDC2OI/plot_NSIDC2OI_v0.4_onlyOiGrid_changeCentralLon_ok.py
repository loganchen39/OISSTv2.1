...
# grid = fig.add_gridspec(nrows=2, ncols=1, height_ratios=[0.55, 0.45], hspace=-0.08, wspace=0.125)
...
year = [2014]
dofy = [74, 258]
title_dofy = ['Mar 15, 2014', 'Sep 15, 2014']
...
for i_yr in range(len(year)):
  ...
  # fig.suptitle('NSIDC SIC to OI-grid ' + str_year, fontsize=18, y=0.80)  # y initially 0.92
    ...
    for i_dy in range(len(dofy)):
        ...
        ax[i_dy].set_extent([-180, 180, -90, 90], ccrs.PlateCarree())
        ax[i_dy].set_global()
    ...
    ct = [0]*2
    for i_dy in range(len(dofy)):
        ct[i_dy] = ds_ns2oi['cdr_seaice_conc'][dofy[i_dy]-1, :, :].plot.contourf(ax=ax[i_dy], vmin=0.0, vmax=1.0  \
            , levels=11, cmap=cmap, add_colorbar=False, transform=ccrs.PlateCarree(), add_labels=False)
    
    cbar = plt.colorbar(ct[1], ax=ax, orientation='horizontal', shrink=0.5, aspect=20, pad=0.05, extend='both'  \
