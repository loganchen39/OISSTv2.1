...
for i_yr in range(len(year)):
    ...
    fig.suptitle('SIC difference bwtween CDO-interp and NCO-interp ' + str_year, fontsize=18, y=0.80)  # y initially 0.92

    fn_ns2oi = str_year + '_sic_nsidc2oi.nc'
    ds_ns2oi_cdo = xr.open_dataset(filename_or_obj=DIR_NS+'/NSIDC2OI/interpByCDO/'+fn_ns2oi, mask_and_scale=True)
    ds_ns2oi_nco = xr.open_dataset(filename_or_obj=DIR_NS+'/NSIDC2OI/interpByNCO/maskZero/'+fn_ns2oi, mask_and_scale=True)
    da_sic_diff = ds_ns2oi_cdo['cdr_seaice_conc'] - ds_ns2oi_nco['cdr_seaice_conc']
    ...
    for i_dy in range(len(dofy)):
        ...
        ax[i_dy].set_title(label='SIC diff', loc='left', fontsize=10, y=1.0, pad=6.0)
    ...
    for i_dy in range(len(dofy)):
      ...
      #     , levels=11, cmap=cmap, add_colorbar=False, transform=proj, add_labels=False)
        ct[i_dy] = da_sic_diff[dofy[i_dy], :, :].plot.contourf(ax=ax[i_dy], vmin=-0.05, vmax=0.05  \
            , levels=6, cmap=cmap, add_colorbar=False, transform=proj, add_labels=False)   
  ...
  # plt.show()
    fn_fig = 'NSIDC2OI_SIC_diff_' + str(year[i_yr]) + '.png'
    plt.savefig('./' + fn_fig)
