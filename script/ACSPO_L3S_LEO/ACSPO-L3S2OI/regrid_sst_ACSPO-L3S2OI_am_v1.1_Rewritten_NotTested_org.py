  ...
  # daily_sst_num_l3s2oi_final.fill(0)
    ...
    for (id_fn, fn) in enumerate(fns, start=0):  # 0: day; 1: night
      ...
      # daily_sst_num_l3s2oi_final[:, :, id_fn] = daily_sst_num_l3s2oi_1rec
        ...
        # to netCDF
        ds_daily = xr.merge([da_daily_sst_avg_day, da_daily_sst_num_day])
        ds_daily.attrs = {}  # or it'll have unwanted attr from da_daily_sst_avg_day
        if id_fn == 0:
            fn_daily = str_date+'_AM_D_ACSPO-L3S2OI.nc'
        else:
            fn_daily = str_date+'_AM_N_ACSPO-L3S2OI.nc'
        ds_daily.to_netcdf(DIR_L3S+'/l3s2oi/am/'+fn_daily, encoding={'lat': {'_FillValue': None}, 'lon': {'_FillValue': None}})


        # to 2D OI-grid binary
        if id_fn == 0:
            fn_daily_bin_2d = str_date+'_AM_D_ACSPO-L3S2OI_2D.bin'
        else:
            fn_daily_bin_2d = str_date+'_AM_N_ACSPO-L3S2OI_2D.bin'
        fl = open(DIR_L3S+'/l3s2oi/am/'+fn_daily_bin_2d, 'wb')
        ...
        # to 1D super-obs
        if id_fn == 0:
            fn_daily_bin_1d = str_date+'_AM_D_ACSPO-L3S2OI_1D.bin'
        else:
            fn_daily_bin_1d = str_date+'_AM_N_ACSPO-L3S2OI_1D.bin'
        ctype = 'dsatam  '
        fort.write_1d_sat_sst_super_obs(DIR_L3S+'/l3s2oi/am/'+fn_daily_bin_1d  \
